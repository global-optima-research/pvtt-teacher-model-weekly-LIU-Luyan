...
后续 **分割路由 Agent** 的完整 VOS pipeline包含三个环节：
...
**本周工作与上述框架的关系：** 本周仅评估 **SAM3 作为"关键帧文本定位模块"** 的单帧分割能力，**尚未接入时序传播**；因此属于pipeline的上游能力摸底，而非端到端全帧推理。
...
**整个测试集所有帧的IoU分布直方图——双峰(U 形)分布**

**右侧(IoU 0.8~1.0):** 绝大多数帧分割得很好

**左侧(IoU ≈ 0):** 有 50 多帧完全失败(模型直接没分割出来或分错)

中间(0.1~0.6)分布比较平,数量都不多
...
| `0013-handbag3_scene01/` | 2/8 | 0.927 | 5/8 | 0.918（仍剩 3 帧失败运动模糊） |
...
**小结：** 23 帧失败中 **20 帧可通过 prompt 细化或略降阈值恢复**；剩余 **3 帧**（`0013-handbag3_scene01` 的 frame_02/03/05）在现有 prompt 与阈值组合下仍无法产出 mask，需后续引入框提示或 VLM 辅助选词。该排查流程也印证了分割路由 Agent 中 **「品类 prompt 需结合 VLM 识别具体包型（handbag / sling bag / backpack）」** 的必要性。

## 5. 结论与发现

### 5.1 SAM3 复现结论

1. **关键帧定位整体可用：** 超过半数采样帧 IoU ≥ 0.7，SAM3 可作为完整 VOS pipeline 中 **关键帧文本定位模块** 的候选后端。
...
4. **与 SAM2 等时序模型的互补关系：** SAM3 的优势在于 **零样本文本定位**（无需首帧点击/框选）。在完整分割pipeline中，预期组合方式为：**SAM3 在关键帧上文本定位 → SAM2/Cutie 等向全视频其余帧传播 mask**，最终输出逐帧 mask 序列。
...
基于本周实验，后续 **分割路由 Agent** 的 VOS pipeline需明确：**所有帧最终都要有 mask**；路由决定的是各模块的分工，而非跳过某些帧。pipeline输出为 **mask 视频**，不包含编辑/替换步骤。
