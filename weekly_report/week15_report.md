...
**本周工作与上述框架的关系：** 本周仅评估 **SAM3 作为"关键帧文本定位模块"** 的单帧分割能力，**尚未接入时序传播**；因此属于完整管线的上游能力摸底，而非端到端全帧推理。

**本周 SAM3 复现具体设置：**

- **模型：** 基于 `Sam3Processor` 文本 grounding 头，对每帧独立做图像级分割（无视频时序传播）。
- **Prompt 设计：** 从文件名解析品类prompt（如 `watch → wristwatch`）。
- **评测协议（方案 A，稀疏采样）：** 每视频**均匀抽取 8 帧**做 SAM3 推理并与 GT 算 IoU——**并非对全视频逐帧推理**。这样做的原因是：（1）PVTT 单视频 GT 可达数百帧，全帧跑 SAM3 算力开销大；（2）8 帧均匀采样足以反映各品类/场景下的分割质量分布，便于与前期 demo 对齐、快速迭代。该 IoU 指标衡量的是**关键帧定位精度**，不能直接等同于接入时序传播后的全视频 J&F。
